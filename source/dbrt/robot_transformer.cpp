...
RobotTransformer::RobotTransformer(
    const std::shared_ptr<RobotTransformsProvider>& transforms_provider)
    : transforms_provider_(transforms_provider),
    ...
    const
{
    ...
    {
        ROS_WARN(
            "Trying to get transforms, but joint values have not been set");
        return transforms_;
    }
    if (transforms_.empty())
    {
        // Use method from robot_state_pub::RobotStatePublisher to get tfs
        transforms_provider_->get_all_transforms(
            joints_, fake_time_, fake_prefix_, transforms_);
    }
    return transforms_;
}
...
void RobotTransformer::set_lookup_provider_() const
{
    ...
    {
        bool ok =
            lookup_provider_.setTransform(transforms[i], "robot_transformer");
        if (!ok)
        {
...
void RobotTransformer::lookup_transform(
