...
ImageVisualizer::ImageVisualizer(const int& n_rows, const int& n_cols)
    : n_rows_(n_rows), n_cols_(n_cols)
{
    ...
    for (int row = 0; row < n_rows_; row++)
        ...
        {
            ((uchar*)(((IplImage*)(image_))->imageData +
                      row * ((IplImage*)(image_))->widthStep))[col * 3 + 0] = 0;
            ((uchar*)(((IplImage*)(image_))->imageData +
                      row * ((IplImage*)(image_))->widthStep))[col * 3 + 1] = 0;
            ((uchar*)(((IplImage*)(image_))->imageData +
                      row * ((IplImage*)(image_))->widthStep))[col * 3 + 2] = 0;
        }
}
...
void ImageVisualizer::set_image(const Eigen::MatrixXd& image,
...
void ImageVisualizer::set_image(const std::vector<float>& image,
                                const float& min_value,
                                const float& max_value,
                                const bool& invert_image)
{
    ...
    for (int row = 0; row < n_rows_; row++)
        ...
        {
            ((uchar*)(((IplImage*)(image_))->imageData +
                      row * ((IplImage*)(image_))->widthStep))[col * 3 + 0] =
                display_image[row * n_cols_ + col];

            ((uchar*)(((IplImage*)(image_))->imageData +
                      row * ((IplImage*)(image_))->widthStep))[col * 3 + 1] =
                display_image[row * n_cols_ + col];

            ((uchar*)(((IplImage*)(image_))->imageData +
                      row * ((IplImage*)(image_))->widthStep))[col * 3 + 2] =
                display_image[row * n_cols_ + col];
        }
}

void ImageVisualizer::add_points(const std::vector<Eigen::Vector3f>& points,
                                 ...
                                 const std::vector<float>& colors)
{
    ...
    for (size_t i = 0; i < points.size(); i++)
    {
        ...
        if (colors.size() == 0) new_colors[i] = point(2);
    }
    ...
    const std::vector<float>& colors)
{
    ...
    for (size_t row = 0; row < points.rows(); row++)
        for (size_t col = 0; col < points.cols(); col++)
        {
            ...
            Vector2i index =
                dbot::hf::CartCoord2ImageIndex(point, camera_matrix);

            if (!(index(0) > 479 || index(1) > 639 || index(0) < 0 ||
                  index(1) < 0))
            {
                point_indices[row * points.cols() + col] =
                    index(0) * n_cols_ + index(1);
...
void ImageVisualizer::add_points(const std::vector<int>& point_indices,
                                 const std::vector<float>& colors)
{
    ...
    //	std::cout << "Min and max of generated depth " << min << " " << max <<
    //std::endl;
    ...
    for (int i = 0; i < int(point_indices.size()); i++)
    {
        ...
        ((uchar*)(((IplImage*)(image_))->imageData +
                  row * ((IplImage*)(image_))->widthStep))[col * 3 + 0] =
            (1 - (new_colors[i] - min) / (max - min)) * 255.;
    }
}

void ImageVisualizer::add_points(const Eigen::VectorXd& depth_points)
{
    // renormalize colors -----------------------------
    float max = -numeric_limits<float>::max();
    float min = numeric_limits<float>::max();
    for (int i = 0; i < int(depth_points.size()); i++)
    {
        if (!std::isfinite(depth_points[i])) continue;

        min = depth_points[i] < min ? depth_points[i] : min;
        max = depth_points[i] > max ? depth_points[i] : max;
    }
    if (min == max) min = 0;
    ...
    for (int i = 0; i < int(depth_points.size()); i++)
    {
        ...
        ((uchar*)(((IplImage*)(image_))->imageData +
                  row * ((IplImage*)(image_))->widthStep))[col * 3 + 2] =
            (depth_points[i] - min) / (max - min) * 255.;
        ...
        ((uchar*)(((IplImage*)(image_))->imageData +
                  row * ((IplImage*)(image_))->widthStep))[col * 3 + 0] =
            (1 - (depth_points[i] - min) / (max - min)) * 255.;
    }
}

char ImageVisualizer::show_image(const std::string& window_name,
...
void ImageVisualizer::Cart2Index(const Eigen::Vector3f& cart,
                                 const Eigen::Matrix3f& camera_matrix,
                                 int& row,
                                 int& col) const
{
